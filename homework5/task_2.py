...
def repeats(our_str):
    """
    Функция считает сколько раз каждый символ уже встречался в переданной строке
    и добавляет через постфикс формата _n сколько раз символ уже встречался в переданной строке.
    Например (Ввод --> Вывод): 'letter' --> l_1e_1t_1t_2e_2_r_1
    :param our_str: исходная строка.
    :return: новая строка, в которой для каждого символа через постфикс формата _n
    выводится сколько раз символ уже встречался в исходной строке.
    """
    letters_dict = {}
    new_str = ''
    for n in our_str:
        new_str += f'{n}_{letters_dict.setdefault(n, 0) + 1}'
        letters_dict[n] += 1
    return new_str
